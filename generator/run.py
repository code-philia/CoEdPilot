...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(0, args.num_train_epochs):
            ...
            for batch in bar:
                ...
                loss,_,_ = model(source_ids=source_ids,source_mask=source_mask,target_ids=target_ids,target_mask=target_mask,args=args)
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        _,loss,num = model(source_ids=source_ids,source_mask=source_mask,
                                           target_ids=target_ids,target_mask=target_mask,args=args)     
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            with open(os.path.join(args.output_dir, "test_{}_pred_gold.json".format(idx)), 'w') as f:
                json.dump(output_dict, f, indent=2)

            (goldMap, predictionMap) = bleu.computeMaps_multiple(os.path.join(args.output_dir, "test_{}_pred_gold.json".format(idx)), args.beam_size) 
            dev_bleu=round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
            logger.info("  %s = %s "%("bleu-4",str(dev_bleu)))
            logger.info("  "+"*"*20)           
