...
def main(args):
    ...
    for sample in dataset:
        code_tokens = ""
        for label, line_of_code in zip(sample["edit_labels"], sample["code_window"]):
            code_tokens += f"{label} {line_of_code}"
    ...
    val_dataset_to_idx = train_dataset_to_idx + int(
        len(transformed_dataset) * args.val_ratio
    )
    test_dataset_to_idx = val_dataset_to_idx + int(
        len(transformed_dataset) * args.test_ratio
    )
