...
def list_files_in_directory(git_repo_path: str, sha: str,
...
def main(lang: str, recalculate_dep_score: bool, test_only: bool, debug_mode: bool,
    ...
    for mode in ["train", "dev", "test"]:
        ...
        if "dependency_score" not in dataset[0].keys(
        ) or recalculate_dep_score:
    ...
    # dataset
    X_train = [val_dataset[idx]["dependency_score"][:1] + [val_embedding_similiarity[idx]]
               for idx in range(len(val_embedding_similiarity))]
    y_train = [val_dataset[idx]["label"]
               for idx in range(len(val_embedding_similiarity))]
    X_test = [test_dataset[idx]["dependency_score"][:1] + [test_embedding_similiarity[idx]]
