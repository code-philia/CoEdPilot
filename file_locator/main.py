...
def list_files_in_directory(git_repo_path: str, sha: str, user_name: str) -> list[str]:
    ...
    if os.path.exists(git_repo_path) == False:
        ...
        command = (
            f"git clone https://github.com/{user_name}/{proj_name}.git {git_repo_path}"
        )
        try:
...
def main(
    ...
    y_train = [
        val_dataset[idx]["label"] for idx in range(len(val_embedding_similiarity))
    ]
    X_test = [
    ...
    y_test = [
        test_dataset[idx]["label"] for idx in range(len(test_embedding_similiarity))
    ]
